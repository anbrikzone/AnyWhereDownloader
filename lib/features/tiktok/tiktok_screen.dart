...
import 'tiktok_controller.dart';

/// Modeled closely on `YouTubeScreen`, minus the adaptive/merge-path UI
/// (pause/cancel row, download-phase label) — TikTok's formats are always
/// muxed (see `TikTokExtractor`), so pause/resume always applies and there's
/// no "video/audio/merging" phase to show.
class TikTokScreen extends ConsumerStatefulWidget {
  const TikTokScreen({super.key, this.initialUrl});
...
class _TikTokScreenState extends ConsumerState<TikTokScreen>
  ...
  final _extractor = TikTokExtractor();
  ...
  Future<void> _checkClipboard() async {
    ...
    final busy = ref.read(tikTokControllerProvider).busy;
    if (busy) return;
  ...
  Future<void> _onFetchPressed() async {
    final controller = ref.read(tikTokControllerProvider.notifier);
  ...
  Widget build(BuildContext context) {
    ...
    final state = ref.watch(tikTokControllerProvider);
    final controller = ref.read(tikTokControllerProvider.notifier);

    ref.listen(tikTokControllerProvider, (previous, next) {
...
String _progressLabel(AppLocalizations l10n, TikTokState state) {
